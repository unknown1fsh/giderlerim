      ...
      <PageHeader
        ...
        eylem={
          <div className="flex items-center gap-2 rounded-lg border border-border bg-bg-secondary px-3 py-2">
            <button onClick={oncekiAy} className="text-text-muted hover:text-text-primary">
              <ChevronLeft className="h-4 w-4" />
            </button>
            <span className="min-w-[120px] text-center text-sm font-medium text-text-primary">
              {AYLAR[ay - 1]} {yil}
            </span>
            <button onClick={sonrakiAy} className="text-text-muted hover:text-text-primary">
      ...
      {dashboard && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            {/* Bütçe Durumları */}
            <div className="lg:col-span-1 rounded-xl border border-border bg-bg-primary p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base font-semibold text-text-primary">Bütçe Durumları</h3>
                <Link href="/butceler" className="text-xs text-accent hover:text-accent-light">
                  Tümünü Gör
                </Link>
              </div>
            ...
            {/* Son Giderler */}
            <div className="lg:col-span-2 rounded-xl border border-border bg-bg-primary p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base font-semibold text-text-primary">Son Harcamalar</h3>
                <Link href="/giderler" className="text-xs text-accent hover:text-accent-light">
                  Tümünü Gör
                </Link>
              </div>
              ...
              {dashboard.kategoriDagilimi.length === 0 ? (
                ...
                <div className="space-y-1">
                  {dashboard.kategoriDagilimi.slice(0, 5).map((item) => (
                    <div
                      key={item.kategoriId}
                      className="flex items-center justify-between rounded-lg p-3 hover:bg-bg-secondary transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-lg">{item.kategoriIkon}</span>
                        <div>
          ...
          {/* AI Insight Teaser */}
          {kullanici?.plan === 'FREE' ? (
            <div className="rounded-xl border border-accent/30 bg-gradient-to-r from-accent/10 to-bg-secondary p-6">
