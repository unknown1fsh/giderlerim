...
export default function UserDropdown() {
  ...
  const adSoyad = kullanici
    ...
    : "Kullanıcı";
  const ilkHarf = kullanici
    ? kullanici.ad.charAt(0).toUpperCase()
    : "K";
  const plan = kullanici?.plan ?? "FREE";
  const planBilgi = planEtiketi[plan] ?? planEtiketi.FREE;
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="mr-3 flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-blue-500 to-purple-600 text-white font-semibold text-sm">
          {kullanici?.avatarUrl ? (
            <Image
              width={44}
              height={44}
        ...
        <span className="mr-1 block max-w-[100px] truncate font-medium text-theme-sm">
      ...
      >
        <div className="mb-1">
          ...
          <span
            className={`mt-1.5 inline-block rounded-full px-2 py-0.5 text-[11px] font-medium ${planBilgi.renk}`}
          >
            {planBilgi.etiket}
          </span>
        </div>
